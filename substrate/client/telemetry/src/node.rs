...
use crate::ConnectionNotifierSender;
...
///    (re-)established. This is used for the "system.connected" message that needs to be sent for
...
pub(crate) struct Node<TTrans: Transport> {
    ...
    /// Notifier senders for when the connection (re-)establishes.
...
impl<TTrans: Transport> NodeSocket<TTrans> {
    fn wait_reconnect() -> NodeSocket<TTrans> {
        let random_delay = rand::thread_rng().gen_range(50..60);
    ...
    TSinkErr: fmt::Debug,
{
    ...
    fn poll_ready(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Result<(), Self::Error>> {
        ...
        // telemetry [`Sink`] returns `Poll::Ready` or `Poll::Pending` quickly and do not perform
        // any heavy computation or blocking operation.
        Poll::Pending
    }
        ...
        if let NodeSocket::Connected(NodeSocketConnected { mut sink, mut buf }) = mem::replace(&mut self.socket, NodeSocket::Poisoned) {
            if sink.poll_ready_unpin(&mut Context::from_waker(futures::task::noop_waker_ref())).is_ready() {
                ...
                if let Err(err) = sink.start_send_unpin(serde_json::to_vec(&item).expect("TelemetryPayload is always serializable; qed")) {
                    log::warn!(target: "telemetry", "⚠️  Telemetry send error (will reconnect): {:?}", err);
                    self.socket = NodeSocket::wait_reconnect();
                } else {
                    self.socket = NodeSocket::Connected(NodeSocketConnected { sink, buf });
                }
            } else {
                // Otherwise, buffer this item and put the sink back.
                buf.push(serde_json::to_vec(&item).expect("TelemetryPayload is always serializable; qed"));
                self.socket = NodeSocket::Connected(NodeSocketConnected { sink, buf });
            }
        } else {
            // Not connected: drop the message (it will not be sent).
            self.socket = NodeSocket::wait_reconnect();
        }
        Ok(())
    }
    ...
    fn poll_flush(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Result<(), Self::Error>> {
        // Ensure all pending connection messages have been sent before considering flush complete.
        let _ = Sink::<TelemetryPayload>::poll_ready(self.as_mut(), cx)?;
        if let NodeSocket::Connected(mut conn) = mem::replace(&mut self.socket, NodeSocket::Poisoned) {
            match conn.sink.poll_flush_unpin(cx) {
                Poll::Ready(Ok(())) => {
                    self.socket = NodeSocket::Connected(conn);
                    Poll::Ready(Ok(()))
                },
                Poll::Ready(Err(err)) => {
                    log::warn!(target: "telemetry", "⚠️  Telemetry flush error (will reconnect): {:?}", err);
                    self.socket = NodeSocket::wait_reconnect();
                    Poll::Ready(Ok(()))
                },
                Poll::Pending => {
    ...
    fn poll_close(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Result<(), Self::Error>> {
        // Attempt to close the underlying connection gracefully.
        if let NodeSocket::Connected(mut conn) = mem::replace(&mut self.socket, NodeSocket::Poisoned) {
            match conn.sink.poll_close_unpin(cx) {
                Poll::Ready(_) | Poll::Pending => { /* ignore result */ },
            }
        }
        // Always return Ready(Ok(())), because we don't want errors during close to propagate.
        Poll::Ready(Ok(()))
    }
}
