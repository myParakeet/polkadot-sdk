...
/// Connection notifier channel types (for telemetry connection events).
pub type ConnectionNotifierSender = mpsc::Sender<()>;
pub type ConnectionNotifierReceiver = mpsc::Receiver<()>;
...
impl TelemetryWorker {
    ...
    pub fn new(buffer_size: usize) -> Result<Self> {
        ...
        // Later transport will be initialized multiple times in `::process_register`, 
        // so it's a convenient way to get an error as early as possible.
        ...
        match input {
            Register::Telemetry { id, endpoints, connection_message } => {
                ...
                for (addr, verbosity) in endpoints {
                    ...
                    // Move any pending connection notifier channels into the node now that it exists
                    pending_connection_notifications.retain(|(addr_b, connection_notifier)| {
                        if *addr_b == addr {
            ...
            Register::Notifier { addresses, connection_notifier } => {
                for addr in addresses {
                    // If the Node has been initialized, push the notifier immediately.
                    // Otherwise, queue it to add when the connection initializes.
    ...
    async fn process_message(
        ...
        let (id, verbosity, payload) = input;
        ...
        let nodes = if let Some(nodes) = node_map.get(&id) {
            ...
            // This is a normal error because the telemetry ID exists before the telemetry is initialized.
            log::trace!(
                target: "telemetry",
...
/// Handle to the [`TelemetryWorker`] that allows initializing the telemetry for a Substrate node.
#[derive(Debug, Clone)]
...
impl Telemetry {
    ...
    /// Make a new clonable handle to this [`Telemetry`]. This is used for reporting telemetry.
